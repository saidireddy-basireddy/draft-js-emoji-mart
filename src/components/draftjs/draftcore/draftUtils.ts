...
const AppendEntity = (
  data: any, stringToAppend: string, entityType: string, editorState: Draft.EditorState
) => {
  ...
  const newContnentState = Draft.Modifier.insertText(
    currentBlock, selection, stringToAppend, undefined, entityKey);
  const newEditorState = Draft.EditorState.push(
    editorState,
    newContnentState, 'insert-characters'
  );
...
export { getTextBetweenTriggerAndCurser, ReplaceCurrentWithEntity, AppendEntity, isCursorAtEndOFBlock, getEntityData }